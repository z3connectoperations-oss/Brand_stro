...
import { relDays, inr, daysLeft } from "@/lib/format";

export function DesignerDashboard({ me }: { me: Employee }) {
  const { db } = useDb();
  const mine = assignedTo(db, me.id);
  const myFeedback = db.feedback.filter((f) => mine.some((d) => d.id === f.deliverableId) && f.status !== "Approved");
  ...
  return (
    <>
      ...
      <KpiGrid cols={5}>
        ...
        <Kpi label="Due today / overdue" value={mine.filter((d) => daysLeft(d.dueDate) <= 0 && d.stage !== "Client approved").length} hint="Tell your TL early, not at the deadline" tone="red" href="/tasks" />
        <Kpi label="In TL review" value={mine.filter((d) => d.stage === "TL review").length} hint="Waiting for sign-off" tone="violet" href="/tasks" />
        <Kpi label="Corrections" value={myFeedback.filter((f) => f.status === "Assigned").length} hint="Reviewed by CRM, assigned by TL" tone="amber" href="/feedback" />
          ...
          <Table head={["Deliverable", "Project", "Stage", "Version", "Due", ""]}>
            {mine.map((d) => (
              <tr key={d.id} className="table-row">
                ...
                <td className="text-slate-600">{db.projects.find((p) => p.id === d.projectId)?.name}</td>
            ...
            {mine.length === 0 && <tr><td colSpan={6} className="py-6 text-center text-slate-500">Nothing assigned yet.</td></tr>}
          </Table>
        </Card>
