...
export default function ReviewQueuePage() {
  ...
  const scoped = me.role === "team-leader" ? db.deliverables.filter((d) => d.team === me.team) : db.deliverables;
  ...
  return (
    <>
      ...
      <div className="my-3"><Chips items={[{ key: "pending", label: "Pending review", count: pending.length }, { key: "ready", label: "Ready for client", count: readyClient.length }, { key: "rework", label: "Had rework" }]} active={filter} onChange={setFilter} /></div>
      <Card padded={false}>
        <Table head={["Deliverable", "Project", "Team", "Designer", "Version", "Client rounds", "Rework", "Due", ""]}>
          {rows.map((d) => { const p = db.projects.find((x) => x.id === d.projectId)!; return (
            <tr key={d.id} className="table-row">
              <td><div className="font-medium">{d.type}</div><Pill tone={stageTone(d.stage)}>{d.stage}</Pill></td>
              <td>{p.name}<div className="text-[11px] text-slate-500">{db.clients.find((c) => c.id === p.clientId)?.name}</div></td>
              ...
              <td>{d.stage === "TL review" ? <Link href={`/review/${d.id}`} className="btn-primary btn-sm">Review</Link> : <Link href={`/tasks/${d.id}`} className="btn-secondary btn-sm">Open</Link>}</td>
            </tr>); })}
