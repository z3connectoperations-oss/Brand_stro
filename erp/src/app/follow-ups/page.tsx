...
import { threads, projectById } from "@/data/projects";
import { clientById } from "@/data/clients";
import { slaState } from "@/lib/selectors";

const SLA = [
  ["WhatsApp — active client project", "Within 4 business hours"],
  ["WhatsApp — general enquiry", "Within 1 business day"],
  ["Email", "Within 1 business day"],
  ["Submission acknowledgement", "Same business day as receipt from production"],
];

export default function FollowUpsPage() {
  const rows = threads.map((t) => ({ t, s: slaState(t) })).sort((a, b) => b.s.hours - a.s.hours);
  ...
  return (
    <>
      ...
      <KpiGrid cols={4}>
        ...
        <Kpi label="SLA adherence (week)" value="86%" hint="Weekly KPI" tone="green" />
      </KpiGrid>
          ...
          <Table head={["Client", "Project", "Channel", "Subject", "Waiting", "SLA", ""]}>
            {rows.map(({ t, s }) => (
              <tr key={t.id} className="table-row">
                ...
                <td className="font-mono text-slate-500">{t.projectId ? projectById(t.projectId)?.code : "—"}</td>
                ...
                <td>{s.state !== "Answered" && <button className="btn-primary btn-sm">Reply & log</button>}</td>
              </tr>
            ))}
          </Table>
        </Card>
        <div className="space-y-4">
          <Card title="Response-time SLAs">
            <ul className="divide-y divide-slate-100">
              {SLA.map(([k, v]) => <li key={k} className="flex justify-between gap-3 py-2 text-[12.5px]"><span className="text-slate-600">{k}</span><span className="text-right font-medium">{v}</span></li>)}
            </ul>
          </Card>
