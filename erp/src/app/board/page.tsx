...
export default function BoardPage() {
  ...
  const rows = db.deliverables.filter((d) => d.stage !== "Closed" && (team === "all" || d.team === team));
  ...
  return (
    <>
      ...
      <div className="mb-3"><Chips items={[{ key: "all", label: "All teams" }, { key: "logo", label: "Logo Team" }, { key: "packaging", label: "Packaging Team" }]} active={team} onChange={setTeam} /></div>
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.title} className="flex min-h-[300px] flex-col rounded-xl border border-slate-200 bg-slate-100/60 p-2">
              <div className="mb-2 flex items-center gap-2 px-1"><span className={`h-2 w-2 rounded-full ${col.tone}`} /><span className="text-[12px] font-semibold text-slate-800">{col.title}</span><span className="ml-auto font-mono text-[11px] text-slate-500">{items.length}</span></div>
              <div className="space-y-2">
                {items.map((d) => {
                  const p = db.projects.find((x) => x.id === d.projectId)!;
                  const late = daysLeft(d.dueDate) < 0;
                  return (
                    <div key={d.id} className="card p-3 hover:border-brand-300">
                      <div className="mb-1 flex items-center justify-between"><PriorityPill p={p.priority} /><span className="text-[10px] font-medium text-slate-400">{teamNames[d.team]}</span></div>
                      <Link href={`/tasks/${d.id}`} className="block"><div className="text-[13px] font-semibold text-slate-900 hover:text-brand-700">{d.type}</div><div className="text-[11.5px] text-slate-500">{db.clients.find((c) => c.id === p.clientId)?.name}</div></Link>
                      <div className="mt-2 flex items-center justify-between"><Pill tone={d.stage === "Scope change pending" ? "red" : "slate"}>{d.stage}</Pill><Avatar employee={byId(d.assigneeId)} size="sm" /></div>
                      ...
                      {canAssign && d.stage === "Awaiting assignment" && <button className="btn-primary btn-sm mt-2 w-full justify-center" onClick={() => setAssign(d.id)}>Assign</button>}
                    </div>
                  );
                })}
              </div>
