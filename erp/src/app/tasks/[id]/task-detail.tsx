...
import { deliverables, projectById, feedback } from "@/data/projects";
...
export function TaskDetail({ id }: { id: string }) {
  ...
  const fb = feedback.filter((f) => f.deliverableId === d.id);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><button className="btn-secondary btn-sm"><Flag size={13} /> Flag scope question to TL</button><button className="btn-secondary btn-sm"><Upload size={13} /> Upload v{d.version + 1}</button><button className="btn-primary btn-sm" disabled={!selfChecked} title={selfChecked ? "" : "Complete the self-check first"}><Send size={13} /> Submit to {tl?.name.split(" ")[0]}</button></>}
      />

      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          {fb.filter((f) => f.status !== "Approved").map((f) => (
            <Card key={f.id} title={`Correction round ${f.round} — from CRM, assigned by ${tl?.name}`} subtitle={`Received ${f.receivedAt.replace("T", " ")} via ${f.channel}`}>
          ...
          <Card title="Brief & direction">
            <Facts cols={2} items={[{ label: "Research brief", value: <Link href="/queue" className="text-brand-700">Drive / 02_R&D / {clientCode}_Brief.pdf</Link> }, { label: d.team === "logo" ? "Sketch concept" : "Brand direction", value: d.team === "logo" ? "Concept 3 (combination mark) — approved by TL" : "Approved identity: ABC saffron + forest green" }, { label: "Must-haves", value: "Scalable, works in black & white, warm palette" }, { label: "Must-avoids", value: "Clip-art leaves, more than 2 typefaces" }]} />
          </Card>
          ...
          <Card title="Versions" subtitle="Never overwrite. Working files stay in your folder; only the TL-approved final goes to 08_Final Files." padded={false}>
            <ul className="divide-y divide-slate-100">
              {versions.map((v, i) => (
                <li key={v} className="flex items-center gap-3 px-4 py-2.5 text-[12.5px]">
                  <FileText size={14} className="text-slate-400" />
                  <code className="flex-1 font-mono text-[11.5px]">{v}</code>
                  {i === versions.length - 1 ? <Pill tone="brand">Current</Pill> : <Pill>Superseded</Pill>}
                </li>
              ))}
              <li className="flex items-center gap-3 bg-slate-50 px-4 py-2.5 text-[12.5px]">
                <Upload size={14} className="text-brand-600" />
                <code className="flex-1 font-mono text-[11.5px] text-slate-500">{nextName}</code>
                <span className="text-[11px] text-slate-500">next upload name</span>
              </li>
            </ul>
          </Card>
        </div>
